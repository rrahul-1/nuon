...
description: 'Let Nuon Install & Manage Nuon in your AWS account'
icon: "cloud-arrow-up"
keywords: ["self-hosted Nuon", "install Nuon", "AWS deployment", "Nuon setup", "BYOC installation"]
---

Nuon BYOC gives you a single-tenant Nuon control plane running in your own AWS account. Nuon uses its own platform to
remotely deploy and manage the control plane as an install — the same mechanism you use to deploy software to your
customers. This means you get automatic upgrades, operational visibility, and lifecycle management,
while retaining full ownership of your data, networking, and compliance posture.

This deployment option is often referred to as Bring Your Own Cloud (BYOC), Cloud-Prem, or Vendor-Managed, Self-Hosted.

Nuon BYOC requires a paid license so to get started, [contact sales](https://nuon.co/contact-sales).
...
flowchart TD
    ...
    subgraph EndCustomers["Your Customers' Clouds"]
        R["Nuon Runner"]
        I["Installs"]
    end
...
When provisioning the stack, provide these secrets:

| Secret               | Value                                  |
| -------------------- | -------------------------------------- |
| `github_app_key`     | Your GitHub App PEM key (base64-encoded if using CloudFormation) |
| `auth_client_secret` | The client secret from your Auth0 SPA  |

<Note>
If you're applying the **CloudFormation** version of the stack, the GitHub App PEM key must be base64-encoded because CloudFormation doesn't preserve newlines in text fields:

```bash
base64 -i your-github-app-key.pem
```

If you're applying the **Terraform** version of the stack, pass the PEM key as-is — Terraform variables preserve newlines.
</Note>
