...
If your template declares any [role enable parameters](#role-enable-parameters), the corresponding IAM role resources
are also added as dependencies — only for the stacks that actually reference them (see
...
| `EnableRunnerManagement`  | Runner Management Role enabled  |
...
stack (e.g., `EnableRunnerProvisioner`, `EnableRunnerDeprovisioner`). These are also treated as reserved parameters in
custom nested stacks.

If your template declares one of these parameters, Nuon will:

1. Pass through the parent stack's parameter value (a `Ref` to the parent parameter).
2. Add the corresponding IAM role resource as a dependency, so the role is created before your stack.

If your template does not declare the parameter, no dependency is added and the role has no effect on your stack.

This allows custom nested stacks to conditionally use IAM roles without creating a hard dependency on them:
...
Conditions:
  ProvisionerEnabled: !Equals [!Ref EnableRunnerProvisioner, 'true']

Resources:
  # Resources that depend on the provisioner role being enabled
  MyResource:
    Type: Custom::Resource
    Condition: ProvisionerEnabled
    Properties:
