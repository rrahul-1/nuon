...
<Warning>
Custom nested stacks are **CloudFormation-only**. The Terraform install module generated for AWS installs does not
include them — `custom_nested_stacks` entries in `stack.toml` are not translated. If you rely on this feature and your
customers want to deploy via Terraform, fork [`install-stacks`](https://github.com/nuonco/install-stacks) and add the
equivalent Terraform resources to your fork.
</Warning>
