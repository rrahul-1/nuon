...
Secret metadata (nott the secret) are defined with a `secrets.toml` and incorporated in the CloudFormation stack deployed by the customer. This allows the customer to enter values when clicking on the stack link provided by the vendor, during the initial step of an install where the runner is created as a VM. Secrets are then stored in the user's AWS Secrets Manager.
