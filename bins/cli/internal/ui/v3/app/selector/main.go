...
func loadApps(ctx context.Context, api nuon.Client, limit, offset int) tea.Cmd {
	return func() tea.Msg {
		apps, hasMore, err := api.GetApps(ctx, &models.GetPaginatedQuery{
			Offset: offset,
			Limit:  limit,
		})
...
func (m model) Init() tea.Cmd {
	return tea.Batch(
		loadApps(m.ctx, m.api, m.limit, m.offset),
		m.spinner.Tick,
	)
}
	...
	case tea.KeyPressMsg:
		...
		case key.Matches(msg, m.keys.Enter):
			if len(m.apps) > 0 {
				...
				if selectedIdx < len(m.apps) {
					m.selectedID = m.apps[selectedIdx].ID
					return m, tea.Quit
				}
			}
			return m, nil
...
func (m model) viewContent() string {
	...
	if m.loading {
		loadingStyle := lipgloss.NewStyle().
			...
			Padding(1, 2)
		content = loadingStyle.Render(fmt.Sprintf("%s Loading apps...", m.spinner.View()))
	} else if len(m.apps) == 0 {
		emptyStyle := lipgloss.NewStyle().
			Foreground(styles.SubtleColor).
			Padding(1, 2)
		content = emptyStyle.Render("No apps found")
	} else {
...
func App(
	...
	if !cfg.Interactive {
		return "", fmt.Errorf("interactive terminal required for app selection; use --app-id flag")
	}

	m := initialModel(ctx, cfg, api, limit, 0)
	p := tea.NewProgram(m)

	finalModel, err := p.Run()
	if err != nil {
		return "", fmt.Errorf("error running app selector: %w", err)
	}
