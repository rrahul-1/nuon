...
type GetInstallActionWorkflowTestService struct {
	fx.In
...
type GetInstallActionWorkflowTestSuite struct {
	tests.BaseDBTestSuite
	app          *fxtest.App
	service      GetInstallActionWorkflowTestService
	router       *gin.Engine
	ctx          context.Context
	testOrg      *app.Org
	testAcc      *app.Account
...
func TestGetInstallActionWorkflowSuite(t *testing.T) {
	if os.Getenv("INTEGRATION") != "true" {
		t.Skip("INTEGRATION is not set, skipping")
		return
	}
	suite.Run(t, new(GetInstallActionWorkflowTestSuite))
}

func (s *GetInstallActionWorkflowTestSuite) SetupSuite() {
...
func (s *GetInstallActionWorkflowTestSuite) SetupTest() {
...
func (s *GetInstallActionWorkflowTestSuite) TearDownSuite() {
	s.app.RequireStop()
}

func (s *GetInstallActionWorkflowTestSuite) setupTestData() {
...
func (s *GetInstallActionWorkflowTestSuite) makeRequest(method, path string, body interface{}) *httptest.ResponseRecorder {
...
func (s *GetInstallActionWorkflowTestSuite) createInstall(appID string) *app.Install {
	install := &app.Install{
...
func (s *GetInstallActionWorkflowTestSuite) createActionWorkflow(appID, name string) *app.ActionWorkflow {
...
func (s *GetInstallActionWorkflowTestSuite) createInstallActionWorkflow(installID, actionID string) *app.InstallActionWorkflow {
...
func (s *GetInstallActionWorkflowTestSuite) TestGetInstallAction() {
	testCases := []struct {
		name         string
		...
		{
			name: "gets install action by id",
			setupFunc: func() (string, string) {
			...
			validateFunc: func(iaw *app.InstallActionWorkflow) {
				...
				require.NotEmpty(s.T(), iaw.InstallID)
				require.NotEmpty(s.T(), iaw.ActionWorkflowID)
				require.NotNil(s.T(), iaw.ActionWorkflow)
				require.Equal(s.T(), "test-action", iaw.ActionWorkflow.Name)
			},
		},
		{
			name: "preloads action workflow",
			setupFunc: func() (string, string) {
				install := s.createInstall(s.testApp.ID)
				action := s.createActionWorkflow(s.testApp.ID, "preload-test")
				iaw := s.createInstallActionWorkflow(install.ID, action.ID)
				return install.ID, iaw.ActionWorkflowID
			},
			expectedCode: http.StatusOK,
			validateFunc: func(iaw *app.InstallActionWorkflow) {
				require.NotNil(s.T(), iaw.ActionWorkflow)
				require.Equal(s.T(), "preload-test", iaw.ActionWorkflow.Name)
				require.Equal(s.T(), s.testApp.ID, iaw.ActionWorkflow.AppID)
			},
		},
		{
		...
		{
			name: "not found for non-existent action",
			setupFunc: func() (string, string) {
				install := s.createInstall(s.testApp.ID)
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			path := fmt.Sprintf("/v1/installs/%s/actions/%s", installID, actionID)
