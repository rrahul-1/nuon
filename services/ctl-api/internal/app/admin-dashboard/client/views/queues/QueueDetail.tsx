              ...
              {(queue.emitters || emittersData?.emitters || []).map((emitter: any) => (
                <tr key={emitter.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                  <td className="whitespace-nowrap px-4 py-3 text-sm">
                    <Link
                      to={`/queues/${id}/emitters/${emitter.id}`}
                      className="text-primary-600 dark:text-primary-400 hover:text-primary-800 dark:hover:text-primary-200 font-mono"
                    >
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-sm">
                    {emitter.status?.status ? (
                      <Badge variant="status" status={getStatus(emitter.status)}>
                        {getStatus(emitter.status)}
                      </Badge>
                    ) : (
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
              {(signals || []).map((signal: any) => (
                <tr key={signal.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                  <td className="whitespace-nowrap px-4 py-3 text-sm">
                    <Link
                      to={`/queues/${id}/signals/${signal.id}`}
                      className="text-primary-600 dark:text-primary-400 hover:text-primary-800 dark:hover:text-primary-200 font-mono"
                    >
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-sm">
                    <Badge variant="status" status={getStatus(signal.status)}>
                      {getStatus(signal.status)}
                    </Badge>
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 text-sm">
        ...
        <div className="mt-2 overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
              ...
              {(!(in_flight_signals || inFlightData?.signals) ||
                (in_flight_signals || inFlightData?.signals || []).length === 0) && (
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                    No in-flight signals
                  </td>
                </tr>
              )}
            </tbody>
