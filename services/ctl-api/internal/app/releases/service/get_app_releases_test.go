...
func (s *ReleasesServiceTestSuite) TestGetAppReleases() {
	testCases := []struct {
		name          string
		...
		{
			name: "success with releases from multiple components",
			setupFunc: func() string {
				cmp1 := s.getSeededComponent(app.ComponentTypeHelmChart)
				ccc1 := s.getSeededConfigConnection(cmp1.ID)
				build1 := s.deps.Seeder.CreateComponentBuild(s.ctx, s.T(), ccc1.ID)

				cmp2 := s.getSeededComponent(app.ComponentTypeTerraformModule)
				ccc2 := s.getSeededConfigConnection(cmp2.ID)
				build2 := s.deps.Seeder.CreateComponentBuild(s.ctx, s.T(), ccc2.ID)
		...
		},
		{
			...
			setupFunc: func() string {
				// Create new app with no releases
				app := s.deps.Seeder.CreateApp(s.ctx, s.T())
				return app.ID
			},
			queryParams:   "",
			expectedCode:  http.StatusOK,
		...
		{
			name: "error with non-existent app",
			setupFunc: func() string {
				return "app_nonexistent12345678901"
			},
			queryParams:   "",
			expectedCode:  http.StatusOK,
			expectedCount: 0,
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			appID := tc.setupFunc()
			path := fmt.Sprintf("/v1/apps/%s/releases%s", appID, tc.queryParams)
...
func (s *ReleasesServiceTestSuite) TestGetAppReleases_OrgIsolation() {
	s.Run("cannot see releases from different org", func() {
		...
		path := fmt.Sprintf("/v1/apps/%s/releases", s.testApp.ID)
