...
type AdminDeleteTestService struct {
	fx.In
...
type AdminDeleteTestSuite struct {
	tests.BaseDBTestSuite
	app           *fxtest.App
	service       AdminDeleteTestService
	router        *gin.Engine
	testOrg       *app.Org
	testAcc       *app.Account
...
func TestAdminDeleteSuite(t *testing.T) {
	...
	suite.Run(t, new(AdminDeleteTestSuite))
}

func (s *AdminDeleteTestSuite) SetupSuite() {
...
func (s *AdminDeleteTestSuite) TearDownSuite() {
	s.app.RequireStop()
}

func (s *AdminDeleteTestSuite) setupTestData() {
...
func (s *AdminDeleteTestSuite) makeRequest(method, path string, body interface{}) *httptest.ResponseRecorder {
	var reqBody []byte
	var err error
	if body != nil {
...
func (s *AdminDeleteTestSuite) TestAdminDeleteRunner() {
	testCases := []struct {
		name               string
		...
		{
			name: "successfully delete runner",
			setupFunc: func() string {
				return s.testRunner.ID
			},
			expectedCode:       http.StatusOK,
			shouldSendSignal:   true,
			expectedSignalType: string(signals.OperationDelete),
		},
		{
		...
		{
			name: "delete deprovisioned runner",
			setupFunc: func() string {
				ctx := context.Background()
