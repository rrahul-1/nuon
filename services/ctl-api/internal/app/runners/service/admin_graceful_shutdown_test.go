...
type AdminGracefulShutdownTestService struct {
	fx.In
...
func TestAdminGracefulShutdownSuite(t *testing.T) {
	...
	suite.Run(t, new(AdminGracefulShutdownTestSuite))
}

func (s *AdminGracefulShutdownTestSuite) SetupSuite() {
...
func (s *AdminGracefulShutdownTestSuite) TearDownSuite() {
	s.app.RequireStop()
}

func (s *AdminGracefulShutdownTestSuite) setupTestData() {
	ctx := context.Background()
	ctx, s.testAcc = s.service.Seeder.EnsureAccount(ctx, s.T())
	s.testOrg = s.service.Seeder.CreateOrg(ctx, s.T())
}

func (s *AdminGracefulShutdownTestSuite) makeRequest(method, path string, body interface{}) *httptest.ResponseRecorder {
	var reqBody []byte
	var err error
	if body != nil {
...
func (s *AdminGracefulShutdownTestSuite) TestAdminGracefulShutDown() {
	testCases := []struct {
		name           string
		runnerID       string
		...
		{
			name:           "graceful shutdown with valid runner ID",
			runnerID:       "rnr123456789012345678901",
			requestBody:    AdminGracefulShutdownRequest{},
		...
		{
			name:           "graceful shutdown with empty body",
		...
		{
			name:           "graceful shutdown with arbitrary runner ID",
			runnerID:       "nonexistent-runner-id-123",
			requestBody:    AdminGracefulShutdownRequest{},
			expectedCode:   http.StatusCreated,
			expectedSignal: true,
			validateFunc: func(runnerID string) {
		...
		{
			name:           "graceful shutdown with empty runner ID",
			runnerID:       "",
			requestBody:    AdminGracefulShutdownRequest{},
			expectedCode:   http.StatusCreated,
			expectedSignal: true,
