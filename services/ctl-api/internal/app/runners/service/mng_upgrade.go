...
import (
	...
	"github.com/nuonco/nuon/services/ctl-api/internal/app/runners/signals/mngupdate"
...
func (s *service) MngUpdate(ctx *gin.Context) {
	...
	if err := s.helpers.EnqueueRunnerSignal(ctx, runner.ID, &mngupdate.Signal{RunnerID: runner.ID}); err != nil {
		ctx.Error(fmt.Errorf("unable to enqueue mng-update signal: %w", err))
		return
	}
