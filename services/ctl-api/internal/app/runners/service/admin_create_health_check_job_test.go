...
type AdminCreateHealthCheckJobTestService struct {
	fx.In
...
type AdminCreateHealthCheckJobTestSuite struct {
	tests.BaseDBTestSuite
	app           *fxtest.App
	service       AdminCreateHealthCheckJobTestService
	router        *gin.Engine
	testOrg       *app.Org
	testAcc       *app.Account
...
func TestAdminCreateHealthCheckJobSuite(t *testing.T) {
	...
	suite.Run(t, new(AdminCreateHealthCheckJobTestSuite))
}

func (s *AdminCreateHealthCheckJobTestSuite) SetupSuite() {
...
func (s *AdminCreateHealthCheckJobTestSuite) TearDownSuite() {
	s.app.RequireStop()
}

func (s *AdminCreateHealthCheckJobTestSuite) setupTestData() {
...
func (s *AdminCreateHealthCheckJobTestSuite) makeRequest(method, path string, body interface{}) *httptest.ResponseRecorder {
	var reqBody []byte
	var err error
	if body != nil {
...
func (s *AdminCreateHealthCheckJobTestSuite) TestAdminCreateHealthCheckJob() {
	testCases := []struct {
		name             string
		...
		{
			name: "successfully create health check job",
			setupFunc: func() string {
			...
			validateFunc: func(runnerID string) {
				...
				err := s.service.DB.WithContext(ctx).
					Where("runner_id = ? AND type = ?", runnerID, app.RunnerJobTypeHealthCheck).
					Order("created_at DESC").
					First(&job).Error
				require.NoError(s.T(), err)

				assert.Equal(s.T(), app.RunnerJobStatusAvailable, job.Status)
				assert.Equal(s.T(), app.RunnerJobTypeHealthCheck, job.Type)
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			rr := s.makeRequest("POST", "/v1/runners/"+runnerID+"/health-check-job", AdminCreateHealthCheckJobRequest{})
