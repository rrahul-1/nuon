...
type AdminReprovisionTestService struct {
	fx.In
...
type AdminReprovisionTestSuite struct {
	tests.BaseDBTestSuite
	app           *fxtest.App
	service       AdminReprovisionTestService
	router        *gin.Engine
	testOrg       *app.Org
	testAcc       *app.Account
...
func TestAdminReprovisionSuite(t *testing.T) {
	if os.Getenv("INTEGRATION") != "true" {
		t.Skip("INTEGRATION is not set, skipping")
		return
	}
	suite.Run(t, new(AdminReprovisionTestSuite))
}

func (s *AdminReprovisionTestSuite) SetupSuite() {
...
func (s *AdminReprovisionTestSuite) SetupTest() {
...
func (s *AdminReprovisionTestSuite) TearDownSuite() {
	s.app.RequireStop()
}

func (s *AdminReprovisionTestSuite) setupTestData() {
...
func (s *AdminReprovisionTestSuite) makeRequest(method, path string, body interface{}) *httptest.ResponseRecorder {
	var reqBody []byte
	var err error
	if body != nil {
...
func (s *AdminReprovisionTestSuite) TestAdminReprovisionRunner() {
	testCases := []struct {
		name               string
		...
		{
			name: "successfully reprovision runner",
			setupFunc: func() string {
				return s.testRunner.ID
			},
			expectedCode:       http.StatusOK,
			shouldSendSignal:   true,
			expectedSignalType: string(signals.OperationReprovision),
		},
		{
