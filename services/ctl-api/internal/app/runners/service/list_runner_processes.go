...
// @Summary				list runner processes
// @Description.markdown	list_runner_processes.md
...
func (s *service) ListRunnerProcesses(ctx *gin.Context) {
	...
	processes, err := s.listRunnerProcesses(ctx, runnerID, org.ID, processType, status, limit, offset)
	if err != nil {
...
func (s *service) listRunnerProcesses(ctx context.Context, runnerID, orgID, processType, status, limit, offset string) ([]app.RunnerProcess, error) {
...
func intFromString(s string, defaultVal int) int {
	var val int
