...
type AdminOfflineCheckTestService struct {
	fx.In
...
func TestAdminOfflineCheckSuite(t *testing.T) {
	...
	suite.Run(t, new(AdminOfflineCheckTestSuite))
}

func (s *AdminOfflineCheckTestSuite) SetupSuite() {
...
func (s *AdminOfflineCheckTestSuite) TearDownSuite() {
	s.app.RequireStop()
}

func (s *AdminOfflineCheckTestSuite) setupTestData() {
	ctx := context.Background()
	ctx, s.testAcc = s.service.Seeder.EnsureAccount(ctx, s.T())
	s.testOrg = s.service.Seeder.CreateOrg(ctx, s.T())
}

func (s *AdminOfflineCheckTestSuite) makeRequest(method, path string, body interface{}) *httptest.ResponseRecorder {
	var reqBody []byte
	var err error
	if body != nil {
...
func (s *AdminOfflineCheckTestSuite) TestAdminOfflineCheck() {
	testCases := []struct {
		name           string
		runnerID       string
		...
		{
			name:           "offline check with valid runner ID",
			runnerID:       "rnr123456789012345678901",
			requestBody:    AdminOfflineCheckRequest{},
		...
		{
			name:           "offline check with empty body",
		...
		{
			name:           "offline check with arbitrary runner ID",
			runnerID:       "nonexistent-runner-id-789",
			requestBody:    AdminOfflineCheckRequest{},
			expectedCode:   http.StatusCreated,
			expectedSignal: true,
			validateFunc: func(runnerID string) {
		...
		{
			name:           "offline check with empty runner ID",
			runnerID:       "",
			requestBody:    AdminOfflineCheckRequest{},
			expectedCode:   http.StatusCreated,
			expectedSignal: true,
