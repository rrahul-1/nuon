...
import (
	...
	"github.com/nuonco/nuon/services/ctl-api/internal/app/runners/signals/mngshutdown"
...
func (s *service) MngShutDown(ctx *gin.Context) {
	...
	if err := s.helpers.EnqueueRunnerSignal(ctx, runner.ID, &mngshutdown.Signal{RunnerID: runner.ID}); err != nil {
		ctx.Error(fmt.Errorf("unable to enqueue mng-shutdown signal: %w", err))
		return
	}
