...
// @Router					/v1/apps/{app_id}/branches/{app_branch_id}/runs/{run_id}/builds [get]
func (s *service) GetAppBranchRunBuilds(ctx *gin.Context) {
	...
	res = s.db.WithContext(ctx).
		...
		Find(&builds)
	if res.Error != nil {
		ctx.Error(fmt.Errorf("unable to get builds: %w", res.Error))
		return
	}
