...
type AdminRestartInstallTestService struct {
	fx.In
...
type AdminRestartInstallTestSuite struct {
	tests.BaseDBTestSuite
	app         *fxtest.App
	service     AdminRestartInstallTestService
	router      *gin.Engine
	testOrg     *app.Org
	testAcc     *app.Account
	testApp     *app.App
...
func TestAdminRestartInstallSuite(t *testing.T) {
	if os.Getenv("INTEGRATION") != "true" {
		t.Skip("INTEGRATION is not set, skipping")
		return
	}
	suite.Run(t, new(AdminRestartInstallTestSuite))
}

func (s *AdminRestartInstallTestSuite) SetupSuite() {
...
func (s *AdminRestartInstallTestSuite) SetupTest() {
...
func (s *AdminRestartInstallTestSuite) TearDownSuite() {
	s.app.RequireStop()
}

func (s *AdminRestartInstallTestSuite) setupTestData() {
...
func (s *AdminRestartInstallTestSuite) makeRequest(method, path string, body interface{}) *httptest.ResponseRecorder {
	var reqBody []byte
	var err error
	if body != nil {
...
func (s *AdminRestartInstallTestSuite) TestRestartInstall() {
	testCases := []struct {
		name             string
		...
		{
			name: "successfully restart install",
			setupFunc: func() string {
				return s.testInstall.ID
			},
			requestBody:    RestartInstallRequest{},
			...
			validateFunc: func(installID string) {
				...
				assert.Equal(s.T(), installrestart.SignalType, capturedSignals[0].Type)
			},
		},
		{
			name: "restart install with empty body",
			setupFunc: func() string {
				return s.testInstall.ID
			},
			requestBody:    nil,
			...
			validateFunc: func(installID string) {
				...
				assert.Equal(s.T(), installrestart.SignalType, sigs[0].Type)
			},
		},
		{
			name: "restart install by name",
			setupFunc: func() string {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			rr := s.makeRequest("POST", "/v1/installs/"+installID+"/admin-restart", tc.requestBody)
