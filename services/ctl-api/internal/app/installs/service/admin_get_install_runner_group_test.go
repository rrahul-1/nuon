...
type AdminGetInstallRunnerGroupTestService struct {
	fx.In
...
type AdminGetInstallRunnerGroupTestSuite struct {
	tests.BaseDBTestSuite
	app           *fxtest.App
	service       AdminGetInstallRunnerGroupTestService
	router        *gin.Engine
	testOrg       *app.Org
	testAcc       *app.Account
	testApp       *app.App
...
func TestAdminGetInstallRunnerGroupSuite(t *testing.T) {
	if os.Getenv("INTEGRATION") != "true" {
		t.Skip("INTEGRATION is not set, skipping")
		return
	}
	suite.Run(t, new(AdminGetInstallRunnerGroupTestSuite))
}

func (s *AdminGetInstallRunnerGroupTestSuite) SetupSuite() {
...
func (s *AdminGetInstallRunnerGroupTestSuite) SetupTest() {
...
func (s *AdminGetInstallRunnerGroupTestSuite) TearDownSuite() {
	s.app.RequireStop()
}

func (s *AdminGetInstallRunnerGroupTestSuite) setupTestData() {
...
func (s *AdminGetInstallRunnerGroupTestSuite) makeRequest(method, path string, body interface{}) *httptest.ResponseRecorder {
	var reqBody []byte
	var err error
	if body != nil {
...
func (s *AdminGetInstallRunnerGroupTestSuite) TestAdminGetInstallRunnerGroup() {
	testCases := []struct {
		name             string
		...
		{
			name: "successfully get runner group via install",
			setupFunc: func() string {
			...
			validateFunc: func(rg *app.RunnerGroup) {
				...
				assert.Equal(s.T(), s.testRunnerGrp.OrgID, rg.OrgID)
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			rr := s.makeRequest("GET", "/v1/installs/"+installID+"/admin-get-runner-group", nil)
